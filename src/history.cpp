...
struct History::Ring {
  ...
  Ring(const char *ns_, const char *key_,
    ...
    , buckets(new (std::nothrow) Bucket[slot_count_]())
  ...
  {
    if (min_persist_sec == 0) return false;
    if (now_epoch < last_persist_epoch) return false;
...
void History::begin()
{
  if (short_ring_) short_ring_->begin();
  if (long_ring_)  long_ring_->begin();
}

void History::record(time_t now_epoch, float value)
{
  if (now_epoch < SYNC_THRESHOLD) return;
